...
#### 为什么Vue组件中的data必须是一个函数，然后return一个对象，而new Vue()实例中data可以直接是一个对象?
   ...
   因为组件是可以复用的，且Js里对象是引用关系。如果组件中的data是一个对象的话，那么作用域就没有隔离了，子组件中的data属性值就会相互受到影响；
   
   如果组件中的data选项是一个函数，那么每个实例可以维护一份被返回对象的独立的拷贝，组件实例之间的data属性值不会互相影响；而new Vue的实例是不会被复用的，所以不存在引用对象的问题。
...
#### vue是如何检测数组的变化?
   ...
   首先使用函数劫持的方式，重写了数组的方法；Vue将data中的数组进行了原型链重写，指向了自己定义的数组原型方法，这样当调用数组api时，就可以通知依赖更新，
   
   如果数组中包含着引用类型，会对数组中的引用类型再次进行观测。
